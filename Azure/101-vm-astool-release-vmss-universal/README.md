# Deployment of a VM Scaleset  (Linux or Windows) running ASTool 
...
the number of instances in the scaleset, by default the value is 2:
...
With 2 instances running with the same configuration in the same scaleset, it improves the reliability of ASTool if for instance ASTool is configured to stream towards Live  ingestion point in Azure Media Services (Push or PullPush features) . In case of a maintenance on one virtual machine, the second machine will be still running.
...
# TESTING THE VIRTUAL MACHINE SCALE SET:
Once the VM has been deployed, you can open a remote session with the VM.

For instance for Linux VM:

     ssh -p 5000X (X: 0 to 5) VMAdmin@vmubus001.eastus2.cloudapp.azure.com

For Windows Server VM:

     mstsc /admin /v:vmwins001.eastus2.cloudapp.azure.com:5000X (X: 0 to 5)
...
With 2 instances running with the same configuration in the same scaleset, it's valuable to test the reliability of Azure Media Services ingestion point. You can configure the virtual machines in the scale set to stream towards a Live ingestion point in Azure Media Services (Push or PullPush features). If you stop ASTool on one virtual machine, the Azure Media Services endpoint associated with the Live channel will keep on streaming provided the second ASTool instance is still running in the second vritual machine.
